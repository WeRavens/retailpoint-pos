...
export const Reports: React.FC<{ isDarkMode?: boolean }> = ({ isDarkMode = false }) => {
  ...
  return (
    <div className={`p-4 md:p-6 h-full overflow-y-auto transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-gray-50 text-gray-900'}`}>
      <div className="mb-6">
        <h1 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Sales Reports</h1>
        <p className={`${isDarkMode ? 'text-slate-400' : 'text-gray-500'} text-sm`}>Overview of store performance</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className={`p-6 rounded-xl shadow-sm border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Total Revenue</h3>
        ...
        <div className={`p-6 rounded-xl shadow-sm border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Transactions</h3>
          ...
          <p className={`text-sm mt-2 ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>Total orders processed</p>
        </div>

        <div className={`p-6 rounded-xl shadow-sm border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Today's Sales</h3>
          ...
          <p className={`text-sm mt-2 ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>Revenue for {new Date().toLocaleDateString()}</p>
        </div>
      </div>
      ...
      <div className={`p-6 rounded-xl shadow-sm border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'} mb-8`}>
        <h2 className={`text-lg font-bold mb-6 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Daily Sales Trend</h2>
